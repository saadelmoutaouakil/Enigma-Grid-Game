...
/**
 * DiscreteCoordinates assume a standard coordinate system. Assume every unit
 * the coordinate axis are graduated. Now imagine a grid passing by all
 * graduation. We get the set of all discrete coordinates: For example if x, y
 * in [0, 2] we get: (0, 0), (1, 0), (2, 0), (0, 1) (0, 2), (1, 1), (1, 2), (2,
 * 1), (2, 2)
 *
 * DiscreteCoordinate are special Vector that can be assimilated to Cells in a
 * grid in a way DiscreteCoordinate (1, 1) include all vector (x, y) with x, y
 * in [1; 2)
 */
public final class DiscreteCoordinates implements Serializable {
	 ...
	 * 
	 * @param delta
	 *            (Vector): to define the length and the direction of the jump. Not
	 *            null
	 * @return (DiscreteCoordinates): the Coordinates after the jump
	 */
	 ...
	 * 
	 * @param v
	 *            (Vector): the vector to check. Not null
	 ...
	 *            (Vector): the vector to check. Not null
	 * @param c
	 *            (DiscreteCoordinates): the discrete coordinate to compare with
	 * @return (boolean) : true if the given vector is close enough from the given
	 *         coordinate
	 */
